...
export const chachingAPIsURLs: string[] = [
  ...
  // `${environment?.apiUrl}/${roots?.dashboard?.orders?.ordersList}`,
...
export function setOrRemoveCacheRequestURL(
  ...
  const urlToCheck = typeof requestURl === 'string' ? requestURl : requestURl.url;

  // If there are params, append them to the URL
  const url = appendParamsToURL(urlToCheck, params);
  ...
  if (type === cachingEnum.REMOVE) {
    ...
    cachedRequestsURLArr = cachedRequestsURLArr.filter(url => !url.startsWith(url));
    ...
    localStorage.removeItem(url); // You can remove specific or all URLs related to the given URL
    return true;
  }

  // Check if the full URL exists in the cache for ADD operation
  const urlExistsInCache = functionToCheck(url);

  if (urlExistsInCache) {
    if (type === cachingEnum.ADD) {
      // Add URL if it's not already in the cached list
      if (!cachedRequestsURLArr.includes(url)) {
        cachedRequestsURLArr.push(url);
        setItemSessionStorage(sessionStorageEnum.CACHINGREQUESTS, JSON.stringify(cachedRequestsURLArr));
      }
    }
    return true;
